-- ============================================================================
-- FIX (v3): Membership Management RLS + Duplicate Prevention
-- Date: 2026-04-25
-- Corrected column name from 'user_id' to 'id' for profiles table
-- ============================================================================
...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = _user_id AND role::text IN ('admin', 'owner')
  )
  OR EXISTS (
...
DROP POLICY IF EXISTS "Authenticated insert memberships" ON public.customer_memberships;
